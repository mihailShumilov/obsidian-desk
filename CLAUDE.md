...
- **Gap I3 CLOSED**: `finalize_settlement` is gated by `market.keeper_authority` AND on-chain SPV merkle inclusion verifier (`programs/obsidian-core/src/spv.rs`, 4 host-runnable tests). `MatchRecord.spv_verified` flag is true only when the on-chain verifier accepted the merkle path.
- **Gap I4 CLOSED on-chain**: `BtcSettleApproval` PDA at `(b"btc_approval", order_pubkey)` + `approve_btc_settlement` (seller-signed) / `consume_btc_approval` (keeper-only) instructions. Replay-safe (one-shot via init), expiry-bound, amount-capped. Keeper integration: `keeper/src/poll.ts::consumeBtcApproval`.
