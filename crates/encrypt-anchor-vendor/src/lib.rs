...
//
// VENDORED for ObsidianDesk — closes gap E2-residual.
//
// Diff vs upstream (rev dadfff8c, chains/solana/program-sdk/anchor):
//   - `invoke_execute_graph`, `execute_graph`, and `execute_registered_graph`
//     now preserve the outer-tx `is_signer` AND `is_writable` flags from each
//     incoming AccountInfo, instead of hardcoding `AccountMeta::new(_, false)`.
//   - This is required for graphs whose outputs are fresh keypair accounts
//     that need to be signers in the outer tx — without preserving the flag,
//     the inner system_program create-account CPI fails with
//     "signer privilege escalated" (or, in pre-allocated mode, "writable
//      privilege escalated").
//   - Single-line diff at each call site, marked with the comment
//     `// vendored: was AccountMeta::new(...,false)`.
...
impl<'info> EncryptCpi for EncryptContext<'info> {
    ...
    fn invoke_execute_graph<'b>(
        ...
        for acct in encrypt_execute_accounts {
            ...
            // outer-tx signer + writable flags so fresh-keypair output cts
            // remain signers at the inner CPI.
            accounts.push(account_meta_for(acct));
        }
...
/// Build an `AccountMeta` from an `AccountInfo`, preserving both
/// `is_signer` and `is_writable` from the outer transaction. This is the
/// vendored fix for gap E2-residual — the upstream helper hardcoded
/// `AccountMeta::new(_, false)` which silently dropped the signer flag for
/// fresh-keypair output ciphertext accounts.
